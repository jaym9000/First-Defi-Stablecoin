...
contract DSCEngine is ReentrancyGuard {
    ...
    constructor(
        ...
        address dscAddress /* USD Price Feeds */
    ) {
     ...
     * @notice You can partialli liquidate a user.
     * @notice You will get a liquidation bonus for taking the user's funds.
     * @notice This function working assumes the protocol will be roughly 200% overcollatoralized in order for this to work.
     * @notice A known bug would be if the protocol were 100% or less collatoralized, then we wouldn't be abl to incentivize the liquidators.
     * For example if the price of the collateral plummeted before anyone could be liquidated.
     * Follows CEIs: Check, Effects, Interactions
     */
        ...
        nonReentrant
    {
        ...
        uint256 totalCollatoralToRedeem = tokenAmountFromDebtCovered + bonusCollateral;
        _redeemCollateral(user, msg.sender, collateral, totalCollatoralToRedeem);
    ...
    function _redeemCollateral(address from, address to, address tokenCollateralAddress, uint256 amountCollateral)
        private
    {
        s_collateralDeposited[from][tokenCollateralAddress] -= amountCollateral; // automatically reverts if not enough by SOL compiler
    ...
    function _healthFactor(address user) private view returns (uint256) {
        ...
        if (totalDscMinted == 0) {
            return 0; // or another appropriate value when no DSC is minted
        }
